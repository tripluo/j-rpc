...
import jrouter.rpc.router.bytecode.javassist.SpringJavassistObjectFactory;
import jrouter.rpc.router.client.websocket.WebSocketClientActionFactory;
...
public class SpringWebSocketClientActionFactory extends WebSocketClientActionFactory implements
     ...
     */
    public SpringWebSocketClientActionFactory(ObjectPool<Session> pool, Map<String, Object> properties) {
     ...
     * 自动加载接口集合。
     */
    private void registerClientInterfacess() {
        if (registry != null && beanFactory != null && _interfaces_added.compareAndSet(false, true)) {
            ...
            for (Object client : clientInterfaces) {
