 ...
 *
 */
package net.jrouter.rpc.router.bytecode.javassist;
...
/**
 * 提供基于javassist的对象工厂，动态构建调用{@link ActionFactory#invokeAction}方法的接口代理对象。
...
//        CtClass.debugDump = System.getProperty("user.home") + "/Desktop" + "/javaDebug";
    ...
    public <T> T newInstance(Class<T> originalInterface) {
        ...
        try {
            ...
            Class<T> proxyClass = (Class) createInterfaceProxyClass(originalInterface)
     ...
     */
    protected <T> T newInstance(Class<T> originalInterface, Class<T> proxiedClass) throws IllegalAccessException,
    ...
    private CtMethod createProxyMethod(CtClass clazz, Class<?> targetClass, Method method) throws CannotCompileException {
        ...
        if ((typeLength = parameterTypes.length) != 0) {
            for (int i = 0; i < typeLength - 1; i++) {
                body.append(parameterTypes[i].getCanonicalName()).append(" p").append(i);
                body.append(',');
            }
            body.append(parameterTypes[typeLength - 1].getCanonicalName()).append(" p").append(typeLength - 1);
        }
        body.append("){");
